...
def common_stations(dfs):
    """Zwraca listę wspólnych stacji w podanych DataFrame
    Args:
        dfs (list): lista DataFrame z danymi PM2.5
    Returns:
        list: lista wspólnych stacji
    """
    common_cols = set(dfs[0].columns[1:]) # pomijamy kolumnę z datami
    for df in dfs[1:]:
        common_cols &= set(df.columns[1:])

    common_cols = list(common_cols)
    return common_cols

def add_multiindex_to_df(df, cities, provinces):
    """Dodaje MultiIndex do Dataframe
    Args:
        df (pd.DataFrame): DataFrame z danymi PM2.5
        cities (dict): słownik mapujący kody stacji na nazwy miejscowości
        provinces (dict): słownik mapujący kody stacji na nazwy wojewódstw
    Returns:
        pd.DataFrame: DataFrame z MultiIndex
    """
    # Zamiana na MultiIndex
    new_columns = []
